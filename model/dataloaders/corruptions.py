...
def fog(x, severity=1):
    ...
    width, height = x.size
    # mapsize is the smallest power of 2 >= to the max of width and height
    mapsize = 2 ** math.ceil(math.log(max(width, height), 2))
    x = np.array(x) / 255.
    max_val = x.max()
    x += c[0] * plasma_fractal(mapsize=mapsize,wibbledecay=c[1])[:height, :width][..., np.newaxis]
...
def waterdrop(x, severity=1):
    ...
    # Set shader data
    shader.channel_0 = toTexture(x, window.ctx)
