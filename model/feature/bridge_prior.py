...
class ContextAwarePrior:
    ...
    def get_context_aware_prior(self, rel_inds, obj_labels, img_ids):
        priors = []
        for img_id in torch.unique(img_ids):
            img_mask = img_ids == img_id
            img_labels = obj_labels[img_mask]
            
            # 计算图片中实体对的共现次数
            cooccur = torch.zeros(151, 151, device=obj_labels.device)
            for s, o in zip(img_labels, img_labels):
                cooccur[s, o] += 1
            
            # 对图片中的每个关系
            img_rels = rel_inds[img_ids[rel_inds[:, 0]] == img_id]
            for rel in img_rels:
